...
export function SourceCoverSection({
  ...
  const doGen = async (interiorPageIds: string[], promptOverride: string, provider: ImageProvider, model: string) => {
    if (!pickFor || interiorPageIds.length === 0) return;
    setBusy(true); setErr(null);
    try {
      for (const id of interiorPageIds) await sc.gen(id, pickFor, promptOverride, provider, model);
      setPickFor(null);
    }
