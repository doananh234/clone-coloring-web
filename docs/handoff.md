...
- [x] 3. Client: `use-clone-jobs.ts` nhận q/source + keepPreviousData; `jobs-screen.tsx` input state local + debounce 300ms, bỏ lọc client (trừ job nháp local), thêm Select nguồn
...
- Debounce gom thành hook `useDebouncedInput` (`packages/coloring/src/hooks/use-query-param.ts`), dùng cho cả books-screen; sửa race ăn ký tự khi commit của chính mình quay về.
