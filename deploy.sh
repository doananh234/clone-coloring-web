...
# Pre-flight: env files must exist locally (they are git-ignored).
for f in apps/admin/.env.local apps/worker/.env; do
    if [ ! -f "$f" ]; then
        echo "ERROR: $f not found. Copy from the matching .env.example and fill in secrets." >&2
        exit 1
    fi
done
...
ssh ${SSH_OPTS} ${SERVER} "cd ${REMOTE_DIR} && docker compose build --parallel"
...
ssh ${SSH_OPTS} ${SERVER} "cd ${REMOTE_DIR} && docker compose ps --format 'table {{.Name}}\t{{.Status}}\t{{.Ports}}'"
