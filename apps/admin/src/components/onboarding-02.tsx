...
const steps: StepData[] = [
  ...
  },
  {
    ...
    description: "Add teammates by email so everyone can collaborate in real time across projects.",
    icon: IconUsers,
    actionLabel: "Send invites",
  },
  {
    title: "Set up your first project",
    description: "Create a project to organize tasks, docs, and milestones all in one place.",
    icon: IconFolder,
    actionLabel: "Create project",
  },
  {
    title: "Connect your tools",
    description: "Link Slack, GitHub, or Figma to keep your workflow in sync automatically.",
    icon: IconPlug,
    ...
    >
      <div className="flex gap-3">
        <div className="mt-0.5 shrink-0">
          <StepIndicator index={index} isActive={isActive} isCompleted={isCompleted} />
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-3">
            <div className="min-w-0 flex-1">
              <p className={cn("font-medium leading-6", getTitleClass(isCompleted, isActive))}>
                {step.title}
              </p>
              <p
                className={cn(
              ...
              {isActive && (
                <Button className="mt-3" onClick={onComplete} size="sm">
                  <StepIcon aria-hidden="true" className="-ml-0.5 size-4 shrink-0" />
                  {step.actionLabel}
                </Button>
              )}
            </div>
            <StepIcon
              aria-hidden="true"
              className={cn("mt-0.5 size-5 shrink-0", getIconColor(isCompleted, isActive))}
            />
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-background p-4">
      <div className="w-full max-w-xl">
        <div className="mb-6">
          <h3 className="font-semibold text-foreground text-lg">Set up your workspace</h3>
