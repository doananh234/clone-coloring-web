...
import { useIsMobile } from "@vx/core-uikit/hooks";
import {
  Badge,
  Button,
  ChartContainer,
...
function DragHandle({ id }: { id: number }) {
  ...
  return (
    ...
    >
      <FontAwesomeIcon icon={faGripVertical} className="size-3 text-muted-foreground" />
    ...
    cell: ({ row }) => (
      <Badge variant="outline" className="px-1.5 text-muted-foreground">
        {row.original.status === "Done" ? (
          <FontAwesomeIcon icon={faCircleCheck} className="fill-green-500 dark:fill-green-400" />
        ) : (
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger render={<Button variant="outline" size="sm" />}>
              <FontAwesomeIcon icon={faColumns3} data-icon="inline-start" />
              Columns
              <FontAwesomeIcon icon={faChevronDown} data-icon="inline-end" />
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-32">
              {table
                .getAllColumns()
