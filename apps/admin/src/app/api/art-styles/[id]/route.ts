...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const doc = await adminDb.collection("artStyles").doc(id).get();
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (regenerateDirective) {
      ...
      data.generationDirective = buildDirectiveFromProperties(merged);
    }
    ...
    if (newReferenceImageUrls && Array.isArray(newReferenceImageUrls)) {
      ...
      for (let i = 0; i < urls.length; i++) {
        ...
        const key = `assets/art-styles/${id}/ref-${i}.png`;
    ...
    await adminDb.collection("artStyles").doc(id).update(data);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  try {
    await adminDb.collection("artStyles").doc(id).delete();
    return NextResponse.json({ success: true });
  } catch (error) {
    return NextResponse.json({ error: String(error) }, { status: 500 });
  }
