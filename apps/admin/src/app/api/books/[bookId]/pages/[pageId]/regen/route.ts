...
import { frameInstruction, buildRedesignPrompt, pickDifferentCameraView } from "@vx/server-core/ai/prompts";
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (styleRefUrls.length) {
      ...
      // No style → reuse the proven, stronger redesign prompt. A forced cameraView
      // makes "đổi góc" genuinely change the composition; a bigger change % pushes
      // it further from the original. It already includes the KDP frame gate.
      prompt = buildRedesignPrompt(newAngle ? 45 : 30, cameraView ? { cameraView } : {});
    }
